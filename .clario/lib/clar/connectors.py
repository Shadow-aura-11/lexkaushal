# clar.connectors — load agent-built data connectors
# Usage: from clar.connectors import use
#   client = use('google_sheets', spreadsheetId='1abc...')
#   data = client.fetch()
import json, os, importlib.util

WORKSPACE = os.environ.get("CLAR_WORKSPACE", os.getcwd())
CONNECTORS_DIR = os.path.join(WORKSPACE, ".clario", "connectors")


def use(slug, **params):
    """Load a connector by slug and return its Client instance.

    Credentials are injected via CLAR_CRED_{KEY} environment variables
    by the extension before script execution.

    Args:
        slug: Connector slug (directory name under .clario/connectors/)
    ...
    credential = None
    if cred_key:
        env_key = "CLAR_CRED_" + cred_key.upper().replace("-", "_")
        credential = os.environ.get(env_key)

    # Dynamic import client.py
    spec_obj = importlib.util.spec_from_file_location(
        f"clar_connector_{slug}", client_path)
